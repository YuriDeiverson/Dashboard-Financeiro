...
const RecentActivity: React.FC<RecentActivityProps> = ({ transactions, setActivePage }) => {
    ...
    <div className="bg-white/80 backdrop-blur-md rounded-2xl p-6 shadow-[0_6px_24px_rgba(12,12,16,0.06)]">
      <div className="divide-y divide-gray-100">
        {recentTransactions.length > 0 ? (
          recentTransactions.map((t) => <ActivityItem key={t.id} transaction={t} />)
        ) : (
          <p className="text-center text-gray-500 py-6">Nenhuma atividade recente.</p>
        )}
      </div>
    </div>
  );
